...
3. **User Choice:**
   - **"Update Now"** → Downloads and prepares the update
   - **"Later"** → Dismisses the notification (will check again on next launch)
4. **Download:** Progress bar shows download status
5. **Install:** After download, user clicks "Restart to Update"
6. **Restart:** App closes, installs update, and reopens with new version ✨
...
[New version available?]
    ...
    [Show "Restart to Update" button]
           ↓
    [User clicks → App restarts with new version] ✅
```
...
npm run version:patch  # 0.9.7 → 0.9.8
npm run version:minor  # 0.9.7 → 0.10.0
npm run version:major  # 0.9.7 → 1.0.0
...
# 4. Launch v0.9.7 → Should detect v0.9.8 available
```
...
4. **Download:** Click "Update Now" → Progress bar appears
5. **Install:** Click "Restart to Update" → App closes
6. **Verify:** App reopens with new version

**Check version:**
- Menu Bar Icon → About Stories
...
- ✅ **Code signature** - DMG/ZIP must be signed
- ✅ **Checksums** - SHA512 hash validation
- ✅ **HTTPS** - All downloads over secure connection
- ✅ **Same publisher** - Certificate must match
...
// ↑ Only these IPC channels are exposed
```
...
**For Developers:**
- Build → Notarize → Publish to GitHub
